...
unsigned int getsI2C_1(unsigned char *rdptr, unsigned char Length)
{
  ...
  {
  	//get a single byte
  	if (getI2C_1(rdptr)!=0) return(1);		
    rdptr++;
    
    //Acknowledge until all read
	if(Length) if(AckI2C_1()!=0) return(2);		
  }
  return (0);
}
...
unsigned int LDByteReadI2C_1(unsigned char I2C_address, unsigned char Register_address, unsigned char *Data, unsigned char Length)
{
    ...
    if (IdleI2C_1()) return (1);
    if (StartI2C_1()) return (2);
    if (WriteI2C_1(I2C_address)) return (3);
    if (ACKCheck_1(&ACKstatus)) return (4);
    if (WriteI2C_1(Register_address)) return (3);
    if (ACKCheck_1(&ACKstatus)) return (4);
    if (RestartI2C_1()) return (6);
    if (WriteI2C_1(I2C_address | 0x01)) return (3);
    if (ACKCheck_1(&ACKstatus)) return (4);
    if (getsI2C_1(Data, Length)) return (7);
    if (NotAckI2C_1()) return (8);
    if (StopI2C_1()) return (5);
